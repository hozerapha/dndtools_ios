//  DiceSceneController.swift
//
//  The production 3D dice engine: procedural die geometry, SceneKit physics,
//  rest detection, face reading, d100 pairing, glow shells, and the magnifier
//  cameras — plus the UIViewRepresentable hosts (SceneKitView, MagnifierView).
//  Extracted from Dice3DPlaygroundView.swift (roadmap item 14a) so the Quick
//  Roll mini tray can spin up its own controller instance without dragging
//  the legacy sandbox view along.
...
/// How the main camera frames the tray.
/// - `.standard`: the dice tab's full tray view.
/// - `.compact`: closer and near-top-down, for the Quick Roll mini tray —
///   1–2 dice rendered inside a small card need to read large, but the whole
///   floor must stay in frame because dice can settle in any corner.
enum TrayFraming {
    case standard
    case compact
}
...
final class DiceSceneController: NSObject {
    ...
    private let framing: TrayFraming

    init(framing: TrayFraming = .standard) {
    ...
    private func setupScene() {
        ...
        case .compact:
            // Calmer, floatier roll for the small viewport: a slower world
            // clock makes the parabola's apex hesitation legible, and
            // lighter gravity stretches the hang time before the die falls
            // back — the "slows at the top, then gravity wins" feel.
            scene.physicsWorld.speed = 2.4
            scene.physicsWorld.gravity = SCNVector3(0, -7.5, 0)
        }
    ...
    private func setupCamera() {
        ...
        case .standard:
            ...
            // One-shot orientation toward the tray instead of a
            // SCNLookAtConstraint — the constraint would keep snapping the
            // camera back every frame and prevent the user from orbiting via
            // SCNView.allowsCameraControl.
            cameraNode.look(at: defaultCameraTarget)
        case .compact:
            // Straight top-down: no angled walls in frame, the felt fills
            // the card edge-to-edge. At y=17 a 52° FOV spans ±8.3 units at
            // the floor — the 15×15 tray plus walls just fits. The explicit
            // up/front vectors sidestep look(at:)'s degenerate case when
            // forward is exactly -Y (world-up would be parallel).
            cameraNode.camera?.fieldOfView = 52
            cameraNode.position = SCNVector3(0, 17, 0)
            cameraNode.simdLook(
    ...
    /// Quick Roll: glide the main camera in over the settled dice so the
    /// rolled faces fill a small viewport — the throw happens under the wide
    /// compact framing (dice can land anywhere), then this zooms to where
    /// they actually stopped. Frames the dice's centroid; height grows with
    /// their spread so two far-apart dice both stay in shot. Compact trays
    /// only — the dice tab keeps its fixed camera.
    func focusCameraOnSettledDice(duration: TimeInterval = 0.55) {
        guard framing == .compact, !dice.isEmpty else { return }

        var centroid = SIMD3<Float>(0, 0, 0)
        for die in dice {
            centroid += SIMD3<Float>(die.node.presentation.position)
        }
        centroid /= Float(dice.count)

        var spread: Float = 0
        for die in dice {
            let p = SIMD3<Float>(die.node.presentation.position)
            spread = max(spread, simd_distance(p, centroid))
        }

        // h=5 at the compact 52° FOV spans ~4.9 units at the floor — one die
        // (~2 units) with comfortable margin; the spread term backs off when
        // multiple dice scattered.
        let height = max(5.0, spread * 2.0 + 4.0)

        SCNTransaction.begin()
        SCNTransaction.animationDuration = duration
        SCNTransaction.animationTimingFunction = CAMediaTimingFunction(name: .easeInEaseOut)
        // Orientation is already straight down (compact setup), so animating
        // the position alone keeps the pure top-down view through the glide.
        cameraNode.position = SCNVector3(centroid.x, height, centroid.z)
    ...
    private func setupTray() {
        ...
        // Compact: the volume is capped well below the straight-down camera at
        // y=17, so a wild throw can never carry a die past the lens — the
        // ceiling IS the "walls go up farther than the throw" guarantee.
        // Standard keeps the tall volume under its y=22 camera.
        let invisibleWallHeight: Float = framing == .compact ? 10 : 20
    ...
    private func throwDieImpulse(at index: Int) {
        ...
        let spawnSpread: Float = framing == .compact ? 1.0 : 1.5
        node.position = SCNVector3(
            Float.random(in: -spawnSpread...spawnSpread),
            5 + Float(index) * 0.25,
            Float.random(in: -spawnSpread...spawnSpread)
        )
        ...
        switch framing {
        case .compact:
            // Mostly-vertical pop with a gentle drift: enough horizontal to
            // feel thrown (and travel a die-width or two), not enough to
            // cross the tray — the die settles near center so the top-down
            // camera barely has to move. The tumble is stronger than the
            // standard tray's (travel isn't doing randomization work here)
            // and its axis is drawn uniformly, no y-bias, so the spin plane
            // varies roll to roll.
            let spinAxis = simd_normalize(simd_float3(
                Float.random(in: -1...1),
                Float.random(in: -1...1),
                Float.random(in: -1...1)
            ))
            let torque = SCNVector4(
                ...
                Float.random(in: 3.0...5.0)
            )
            body.applyTorque(torque, asImpulse: true)

            let driftAngle = Float.random(in: 0..<(2 * .pi))
            let drift = Float.random(in: 2.0...4.5)
            body.applyForce(SCNVector3(
                cos(driftAngle) * drift,
                Float.random(in: 5...8),
                sin(driftAngle) * drift
            ), asImpulse: true)
        ...
        case .standard:
            ...
            // Random direction with a GUARANTEED minimum horizontal
            // magnitude. The previous per-axis ±10 uniform could land near
            // zero on both axes, lobbing the die straight up — it would drop
            // flat and settle on the same face without ever tumbling.
            let angle = Float.random(in: 0..<(2 * .pi))
            let horizontal = Float.random(in: 6...12)
