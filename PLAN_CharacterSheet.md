...
### Audit verdict (2026-06-13, updated 2026-07-06)
Bundle is SRD-clean at the inventory level: **all 12 SRD 5.2.1 classes and one subclass per class shipped** — 12 classes, 12 subclasses (Champion/Thief/Life Domain/College of Lore/Draconic Sorcery/Circle of the Land/Fiend Patron/Path of the Berserker/Oath of Devotion/Evoker/Warrior of the Open Hand/Hunter — exactly the SRD ones), 9 species (all SRD), 4 backgrounds (all 4 SRD), 45 spells (all SRD), all weapons/armor/conditions/gear, the 2 referenced feats. Attribution notice shipped. Description-text wording should track SRD phrasing (CC-licensed) — never PHB-exclusive wording.
...
- **2026-07-06 — Ranger (12th class) + Hunter (12th subclass) — class catalog complete.** Half-caster (`casterLevelDivisor: 2`) with `preparedFromAll` + a `spellsKnown` table 2 → 14 (honored as the prep cap by the fresh Bard/Sorcerer/Ranger fix in `57fc0aa` — a WIS-8 ranger still gets 2 prepared at L1 instead of the old ability-mod-only clamp); no cantrips (`cantripsKnown` byClassLevel 0); slot table starts at L2, tops at 5th-level slots (identical to paladin at matched levels). Content: **Favored Enemy** always grants **Hunter's Mark** with a scaling free-cast pool (2/3/4/5 per Long Rest, `surfacesAsAction:false` — the on-damage rider chip flows through the existing Hunter's Mark plumbing); Weapon Mastery picks 2 → 3 at L9; Deft Explorer + L9 Expertise as skill-choice pickers; Fighting Style (Archery / Defense / Dueling / Two-Weapon — **no** Great Weapon Fighting for rangers); Extra Attack at L5; **Roving** uses the `speedBonus` field for +10 ft; Nature's Shield / Nature's Veil / Tireless as WIS-mod / Long Rest pools; Relentless Hunter, Precise Hunter, Foe Slayer as text riders. **Hunter:** Hunter's Prey (Colossus Slayer as an opt-in on-hit **+1d8 weapon-typed** rider on the Sneak Attack pattern via `addDamageDice`, once/turn); Escape the Horde, Evasion, and Superior Prey d10 as text. *Known simplifications:* Hunter's Prey / Defensive Tactics / Superior Hunter's Defense each have 3 SRD options — only one authored per tier (Colossus Slayer, Escape the Horde, Evasion); L15 Superior Hunter's Prey damage-die bump is text (Colossus Slayer's die stays 1d8; adjust manually). Tests: `RangerTests` (new file, 13).
- **2026-07-06 — Monk (11th class) + Warrior of the Open Hand (11th subclass), full L1–20 + Focus-pool engine.** Engine additions: `scaledDamage` gained die-SIZE scaling (`dieKind` is now a `LevelScaledValue` — Martial Arts 1d6 → 1d8 → 1d10 → 1d12) + an optional `addAbility` modifier; `abilityRoll` gained `addLevel`; **`FeatureDefinition.resourceCost`** — a feature that SPENDS another feature's pool (groundwork for Battle Master superiority dice / Sorcery Point spends); **`FeatureDefinition.speedBonus`** (unarmored-only, folded into the Speed chip; also ready for Barbarian Fast Movement); and unarmed strikes now add proficiency bonus (weapon-nil attacks were non-proficient — a bug for everyone, fatal for monks). Content: **Focus points** = monk level (short-rest pool, `surfacesAsAction:false`); Flurry of Blows / Patient Defense / Step of the Wind = Bonus-Action Focus spends via `resourceCost`; Martial Arts scaling die + DEX with unarmed attack rows; Unarmored Movement speed bonus (+10 → +30) on the Speed chip; Deflect Attacks = 1d10 + DEX + level Reaction; Stunning Strike = WIS save DC + 1 Focus; Superior Defense = 3 Focus. Open Hand: Wholeness of Body (Martial-Arts-die + WIS heal, WIS-mod/LR pool), Quivering Palm (10d12 force, 4 Focus). *Known simplifications:* Extra Attack, Heightened Focus upgrades, and Focus-DC riders (Open Hand Technique, Stunning Strike's on-save effects) are text; a multiclass monk's Martial Arts die scales by character level. Tests: `MonkTests` (new file, 11); ContentStore/ContentLint tests updated for the new `scaledDamage` shape.
...
- **Classes 12/12 done ✅** (Fighter, Wizard, Rogue, Barbarian, Paladin, Cleric, Bard, Sorcerer, Druid, Warlock, Monk, Ranger) — class catalog complete.
- **Subclasses 12/12 done ✅** (Champion, Thief, Life Domain, College of Lore, Draconic Sorcery, Circle of the Land, Fiend Patron, Path of the Berserker, Oath of Devotion, Evoker, Warrior of the Open Hand, Hunter). NO Battle Master / EK / Arcane Trickster (PHB-only → importable pack only).
...
- **11h. More subclasses — SRD one-per-class ONLY** ✅ (shipped 2026-07-06 — Berserker, Circle of the Land, Warrior of the Open Hand, Oath of Devotion, Hunter, Fiend Patron, Evoker all bundled with their parent classes). Battle Master / Eldritch Knight / Arcane Trickster are PHB-only — importable pack only. (Battle Master's superiority dice still motivate `TriggerCost.resource(id:amount:)` as engine groundwork for the pack, not bundled content.)
