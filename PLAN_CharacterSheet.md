...
| O — Triggered effects & active statuses | **Slice A shipped** — Hex + Hunter's Mark end-to-end. Substrate: `TriggeredEffect` schema (`TriggerCondition.onDamageRoll`, `TriggerEffect.addDamageDice`, `TypedOrMatch.fixed/matchWeapon`, `TriggerLifecycle.persistent(.concentrationEnds)`), `ActiveEffect` + `EffectSource` on the character, `SpellDefinition.grantsTriggeredEffect`. `TriggeredEffectResolver` folds matching automatic riders into weapon damage `ResolvedAction`s with the right `damageType` so Phase N's breakdown renders the split naturally. `Character.startConcentrating(on:grantsEffect:)` + `stopConcentrating()` + `dismissActiveEffect(_:)` keep the concentration ⇄ effects link consistent across all surfaces (SpellCastSheet swap, ConcentrationSaveSheet fail, ConditionsRow Drop pin, EffectsRow badge dismiss). New `EffectsRow` UI in the sticky header. Codable is backwards-compat for both Character (`activeEffects` decodes nil → empty) and SpellDefinition (`grantsTriggeredEffect` decodeIfPresent). **Slice B (next):** `.optIn` chips for Sneak Attack + Divine Smite (requires generalizing `PendingRollStore.followUp` to a list, `turnFlags` for once-per-turn). **Slice C:** `.toggleBeforeRoll` + Rage rounds tracker + GWM. |
...
*Next step: Phase O Slice B — opt-in chips for Sneak Attack and Divine Smite. Generalize `PendingRollStore.followUp` to a `followUps: [ResolvedAction]` list, add `Character.turnFlags: Set<String>` plus a "Start new turn" button on the sheet, extend `TriggerActivation` with `.optIn` and `TriggerCost` (`.spellSlot`, `.oncePerTurn`), wire the resolver to surface chips after the primary roll settles. Slice A already provides the schema scaffolding and the resolver entry point.*
